...
# OI CM5 Setup Script
# Script to install dependencies and configure Raspberry Pi CM5

USER_DIR="/home/$USER"
SYSTEM_SERVICES="${USER_DIR}/oi-pi5-toolkit/system-services"

echo "Starting system setup for CM5..."
...
CORE_PKGS=(git meson ninja-build pkg-config gcc g++ python3-pip curl iw network-manager gpsd gpsd-clients python3-pandas)
...
for program in "${CORE_PKGS[@]}"; do
	...
	PYVER=$(python3 -c "import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}')")
        if [ -f /usr/lib/python${PYVER}/EXTERNALLY-MANAGED ]; then
            ...
            sudo mv /usr/lib/python${PYVER}/EXTERNALLY-MANAGED /usr/lib/python${PYVER}/EXTERNALLY-MANAGED.old
        else
...
sudo pip3 install pymavlink pyserial --break-system-packages
...
# For CM5, change ttyS1 to ttyAMA2
Device = /dev/ttyAMA2
Baud = 115200
[UdpEndpoint doodle]
Mode = Server
...
mkdir -p /home/$USER/gst-rtsp-server
cd /home/$USER/gst-rtsp-server

echo "[INFO] Writing stream.py..."
cat << 'EOF' > /home/$USER/gst-rtsp-server/stream.py
#!/usr/bin/env python3
...
chmod +x /home/$USER/gst-rtsp-server/stream.py
chown -R $USER:$USER /home/$USER/gst-rtsp-server
...
[Service]
ExecStart=/usr/bin/python3 /home/$USER/gst-rtsp-server/stream.py
WorkingDirectory=/home/$USER/gst-rtsp-server
Restart=always
User=$USER
